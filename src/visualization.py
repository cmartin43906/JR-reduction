...
def plot_proxy_sweeps(sweep_dicts):
    ...
    for ax, sweep_params in zip(axes, sweep_dicts):
        ...
        if sweep_key is None:
            raise ValueError("No sweep parameter found. One parameter must be a list.")
...
def plot_spectral(param_sets):
    ...
    for params in param_sets:
        ...
        dt = t[1] - t[0]
        num_samples = len(proxy)

        # fast fourier transform to get values
        centered_signal = proxy - np.mean(proxy)
        fft_coeff = np.fft.rfft(centered_signal)
        freqs = np.fft.rfftfreq(num_samples, d=dt)
        amps = np.abs(fft_coeff)

        # we could plot power here, but i tried it and it makes visual comparison infeasible. so we stick with amplitudes.
